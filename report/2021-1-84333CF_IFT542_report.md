...
> system was targeted at any point, and no reusable attack payload is submitted. The signed
> declaration is in `ETHICS.md`.
...
The plaintext `password` column is dropped. **Evidence: `evidence/task2/07-argon2id-hashes.png`**
...
**Evidence: `evidence/task2/13-tests-green.png`.** The Task 2 files assert the four required
proofs plus the two extra controls:
...
**Evidence: `evidence/task3/15-xss-neutralised.png`** captures both halves — `/dashboard`
...
**Evidence: `evidence/task3/16-csrf-rejected.png`** shows layer 1 as a browser experiences it —
...
first; `16b-csrf-rejected.png` shows those two failing on their own from a Node client, which does
not implement `SameSite`.
...
**Evidence: `evidence/task3/17-cookie-flags.png`** shows the devtools cookie row with HttpOnly,
...
**Evidence: `evidence/task3/18-ssrf-blocked.png`.** `ssrf-guard.test.ts` covers the full matrix
...
**Evidence: `evidence/task3/19-security-headers.png`** captures the production response headers:
...
All were absent in the baseline. The capture must be taken against `npm run build && npm start` —
the strict policy does not apply under `npm run dev`, and any `unsafe-` or `ws:` in a capture means
the development policy was captured by mistake. HSTS is inert over plain HTTP on localhost and is
not claimed as an active control here.
...
**Evidence: `evidence/task3/21-security-logs.png`** shows the three event types appearing live in
...
The signed declaration is in `ETHICS.md` and covers ten clauses: authorisation, containment,
...
Full remediation status per finding, with the control implemented and its new location, is in
`report/task1-threat-model.md` Appendices B (Task 2), C (Task 3) and D (corrective controls).
...
**Evidence: `evidence/task3/22-tests-green.png`.**
...
the long-form operational detail, with every command's real captured output, is
`report/appendix/response-runbook.md`.

The response cycle is captured as text in `evidence/task3/run-output-after.txt` §7–§9: the full
...
that would invalidate every capture in this report. The advisory group covers denial-of-service,
