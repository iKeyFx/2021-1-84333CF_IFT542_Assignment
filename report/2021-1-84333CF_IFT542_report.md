...
SQL directly — and impact is maximal. **Residual 5:** after parameterised binding, user input can
...
**How parameterisation separates data from code.** This is a postgres.js *tagged template*, not a
...
other stacks. Validation is defence in depth here, not the injection fix: the parameterised query
...
in the simulated incident `INC-2026-001`, parameterising the login query stopped new bypasses but
...
|------|----|--------|--------|---|---|------|----------|--------|----------|
| 1 | T1 | S | SQLi auth bypass | 5 | 5 | **25** | Critical | Parameterised queries **(P)**; input validation **(P)**; log rejected/anomalous queries **(D)**; session revocation + runbook **(C)** | 1×5 = **5** |
...
`INC-2026-001`, it had. Parameterising the login query stopped new bypasses; it did nothing about
