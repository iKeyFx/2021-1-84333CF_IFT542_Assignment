...
`evidence/task1/01-login-student.png` and `evidence/task1/01-login-admin.png` (student and admin
login), `02-profile-update.png` (profile update), `03-course-register.png` (course registration),
`04-upload.png` (document upload), `05-admin-courses.png` (admin course management) and
`06-admin-enrolments.png` (admin enrolment management). The flat file-path index of every planted
...
**Evidence: `evidence/task2/08-sqli-no-bypass.png`** records the payload `' OR '1'='1' -- `
...
**Evidence: `evidence/task2/13-tests-green.png`** records the suite as it stood when Task 2 closed
...
`evidence/task3/22-tests-green.png` and itemised in **Appendix B**.
...
**Evidence: `evidence/task3/15-xss-neutralised.png`** shows both halves — `/dashboard`
...
first; `evidence/task3/16b-csrf-rejected.png` shows those two failing on their own from a Node
...
**Evidence: `evidence/task3/19-security-headers.png`** shows the production response headers:
...
| No SameSite on session cookie | T2 | 5 | **Closed** | `HttpOnly; SameSite=Lax; Secure`. Lax not Strict, so inbound links do not appear logged-out while still blocking the cross-site POST CSRF needs. | `lib/session.ts` | `tests/security-headers.test.ts`; `evidence/task3/17-cookie-flags.png` |
